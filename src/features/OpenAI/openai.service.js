                ...
                for (const toolCall of responseMessage.tool_calls) {
                    if (toolCall.function.name === 'update_customer_data') {
                        try {
                            ...
                            // 🛡️ Name Guard: Ignore if AI thinks customer is Sheila (self-identity confusion)
                            let extractedName = data.name || chat.contactName;
                            if (extractedName && (extractedName.toLowerCase().includes('sheila') && extractedName.toLowerCase().includes('araujo'))) {
                                console.log(`⚠️ AI tried to set name to Sheila Araujo. Ignoring name update to prevent identity collision.`);
                                extractedName = chat.contactName; // Revert to old name
                            }
