...
3. **MENSAGENS DE ÁUDIO**:
   - **VOCÊ OUVE ÁUDIOS**: O sistema transcreve áudios automaticamente.
   - Quando vir \`[Transcrição de Áudio]:\`, saiba que é o conteúdo do áudio que o cliente enviou. Responda normalmente ao conteúdo.
                ...
                for (const toolCall of responseMessage.tool_calls) {
                    if (toolCall.function.name === 'update_customer_data') {
                        try {
                            ...
                            // If we have Name + Email but NO CPF, generate a random one
                            const hasIdentity = (data.name || chat.contactName) && (data.email || chat.email);
                            if (hasIdentity && !finalCpf) {
                            ...
                            if (hasIdentity && !chat.tramitacaoCustomerId) {
                                console.log(`🚀 Core data (Name+Email) captured. Using CPF ${finalCpf}. Triggering auto-sync...`);
                                try {
